            ...
            <>
              ...
              <div className="cart-box">
                {get_local !== null
                  ? get_local.map((a, i) => {
                      return (
                        <p style={{ marginTop: "10px" }}>{get_local[i]}</p>
                      );
                    })
                  : null}
              </div>
          ...
          2초이내 구매시 할인
                ...
                <button
                  onClick={() => {
                    ...
                    // } else if (nums == 2) {
                    axios
                      .get("https://liverpooll.github.io/data/data2.JSON")
                      .then((result) => {
                        let player_1 = [...player];
                        let new_player = [...player_1, ...result.data];
                        setPlayer(new_player);
...
const List = (props) => {
  ...
  return (
    <div className="col-md-4">
      <Nav.Link
        onClick={() => {
          navigate("/detail/" + props.player.id);
        }}
        style={{ display: "inline-block" }}
      >
        <img
          src={process.env.PUBLIC_URL + "/" + props.player.id + ".png"}
          width="70%"
          height="200px"
        />
      </Nav.Link>
